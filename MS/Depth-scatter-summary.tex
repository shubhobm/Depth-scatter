...
\begin{Definition}
Given a random variable $\bfX$ following a probability distribution $P$, and a depth function $D(.,.)$, we define an Htped of a point $\bfx: H(\bfx, F) = h(d_\bfx)$ as any function of the data depth so that $h(d_\bfx)$ is monotonically decreasing in $d_\bfx$ and $E_F (H(\bfx, F))^4 < \infty$.
\end{Definition}
