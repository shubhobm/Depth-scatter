...
\begin{Definition}
Given a random variable $\bfX$ following a probability distribution $F$, and a depth function $D(.,.)$, we define Htped of a point $\bfx$ as: $\tilde D(\bfx, F) = h(d_\bfx)$ as any function of the data depth so that $h(d_\bfx)$ is bounded, monotonically decreasing in $d_\bfx$ and $\sup_\bfx (\tilde D(\bfx, F))^4 < \infty$.
\end{Definition}
...
future: test for elliptic symmetry

future: high-dimension, regression scenario
