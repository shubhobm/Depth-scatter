...
This data is from the UCI machine learning repository [], and consists information on 101 demographic and socio-economic variables (like population, racial percentages, percentage of people living in urbal areas etc.)  in 1994 community areas in the United States. We shall attept to build a principal component-based linear regression models to predict one of the variables, the total number of violent crimes per 100K population from others. Specifically, we will be building linear models on increasing numbers of PCs from both normal PCA and depth-based PCA done on the predictors, and will compare the change of $R^2$ values for each group of models. We do this since boxcox transformations on all of the models we consider suggest optimal power transformations of values close to -3.

We keep all the predictors unchanged, while go for an inverse cubic transformation on the response variable. While doing the depth-based PCA, we use projection depth. Both of the methods seem to explain same amounts of variances in all leading PCs (Figure ).
