...
\subsection{Bus data}
This dataset is available in the R package \texttt{rrcov}, and consists of data on images of 218 buses. The 18 variables here correspond to several features related to these images. Here we extend upon the analysis in \cite{maronna06}, pp. 213 to compare 3 different methods of robust PCA (including that using SCM) with our DCM-based method. Similar to the original analysis, we set aside the variable 9 and scale the other variables by dividing with their respective mean absolute deviations (MAD). This is done because all the variables had much larger standard deviations compared to their MADs, and variable 9 had MAD = 0.

For the sake of uniformity, we use projection depth as our fixed depth function while doing depth-based PCA in our data analysis examples. We compare the outputs of classical and depth-based PCA with the following 3 robust methods: Spherical PCA, i.e. PCA based on the SCM, PCs obtained by the ROBPCA algorithm \citep{hubert05}, and the Minimum Covariance Determinant (MCD) estimator \citep{Rousseeuw84leastmedian}. Table \ref{table:bus_table1} gives the proportions variability that are left unexplained by the top $q$ $(= 1,...,6)$ components obtained from all 5 methods. The first PC in classical PCA seems to explain a much higher proportion of variability of original data than robust methods. However, as noted in \cite{maronna06} and \cite{hubert05}, this is a result of the classical variances being inflated due to outliers in the direction of the first principal axis. Among the robust methods, both DCM-based PCA and ROBPCA have 41\% unexplained variance after the first PC is taken into account. The other proportions for these two methods are also very similar.

Table \ref{table:bus_table2} demonstrates why robust methods actually give a better representation of the underlying data structure than classical PCA here. Each of its column lists different quantiles of the squared distance of a sample point from the hyperplane formed by top 3 PCs estimated by the corresponding method. For ROBPCA and PCA based on SCM and projection-DCM, the estimated principal component subspaces are closer to the data for upto 70\% of samples, and the distance only becomes larger for higher quantiles. This means that for classical PCA, estimated basis vectors of the hyperspace get pulled by extreme outlying points, while the influence of these outliers is low for the 3 robust methods. For SCM-PCA and ROBPCA, the percentage of points that have less squared distance than classical PCA actually rises to 80\%. This percentage is 50\% for MCD-based PCA, which suggests that the corresponding 3-dimensional subspace estimated by MCD is possibly not an accurate representation of the truth.
...
\centering
    ...
    ~                   & Classical     & SCM-based & ROBPCA & MCD-based & DCM-based \\\hline 
    1                   & 0.188         & 0.549     & 0.410  & 0.514     & 0.411     \\
    2                   & 0.084         & 0.272     & 0.214  & 0.337     & 0.219     \\
    3                   & 0.044         & 0.182     & 0.121  & 0.227     & 0.117     \\
    4                   & 0.026         & 0.135     & 0.083  & 0.154     & 0.085     \\
    5                   & 0.018         & 0.099     & 0.054  & 0.098     & 0.060     \\
    6                   & 0.012         & 0.069     & 0.036  & 0.070     & 0.044     \\ \hline
    \end{tabular}
    \caption{Unexplained proportions of variability for PCA models with $q$ components for bus data}
...
\centering
    ...
    ~                   & Classical     & SCM-based & ROBPCA & MCD-based & DCM-based \\\hline 
    10\%      & 1.9       & 1.2       & 1.2    & 1.1       & 1.3       \\
    20\%      & 2.3       & 1.6       & 1.6    & 1.4       & 1.6       \\
    30\%      & 2.8       & 1.8       & 1.8    & 1.7       & 1.9       \\
    40\%      & 3.2       & 2.2       & 2.1    & 2.1       & 2.4       \\
    50\%      & 3.7       & 2.6       & 2.5    & 2.9       & 2.7       \\
    60\%      & 4.4       & 3.1       & 3.0    & 5.6       & 3.4       \\
    70\%      & 5.4       & 3.8       & 3.9    & 22.7      & 4.3       \\
    80\%      & 6.5       & 5.2       & 4.8    & 82.5      & 7.2       \\
    90\%      & 8.2       & 9.0       & 10.9   & 286.0     & 22.4      \\
    Max       & 24        & 1037      & 1055   & 1042      & 1036      \\\hline
    \end{tabular}
...
We now apply our method to a high-dimensional dataset and demonstrate its effectiveness in outlier detection. Due to \cite{esbensen94}, this dataset consists of 226 variables and 39 observations. Each observation is a gasoline sample with a certain octane number, and have their NIR absorbance spectra measured in 2 nm intervals between 1102 - 1152 nm. There are 6 outliers here: compounds 25, 26 and 36-39, which contain alcohol. Proportions of \textit{explained} variability by PCs obtained by the two methods are given in Figure \ref{fig:screeplot}. Once again, the first PC in classical PCA explains a larger proportion of variance than the robust method, although the difference is rather small. Second and third PCs obtained by depth PCA explain slightly higher proportions of variability.

In both methods, the first two PCs explains a large amount ($> 90\%$) of underlying variance. But in depth PCA, these PCs are more effective in detecting outliers, which we demonstrate in Figure \ref{fig:distplot}. For any method of PCA with $k$ components on a dataset of $n$ observations and $p$ variables, the \textbf{score distance} (SD) and \textbf{orthogonal distance} (OD) for $i^\text{th}$ observation ($i=1,2,...,n$) are defined as:
$$ SD_i = \sqrt{ \sum_{j=1}^k \frac{s^2_{ij}}{\lambda_j}}; \quad OD_i = \| \bfx_i - P\bfs_i^T \| $$
where $S = (\bfs_1, \ldots ,\bfs_n)^T$ is the $n\times k$ scoring matrix, $P$ the $p\times k$ loading matrix, and $\lambda_1,\ldots ,\lambda_k$ are eigenvalues obtained from the PCA, and $\bfx_1,\ldots,\bfx_n$ are the $n$ observation vectors. We only consider the first 2 PCs here, so $k$ is set to 2. From a practical standpoint, $SD_i$ can be interpreted as a weighted norm of the projection of the $i^\text{th}$ point on the hyperplane formed by first $k$ principal components, and $OD_i$ the orthogonal distance of point $i$ from that hyperplane. For outlier detection, following \cite{hubert05} we set the upper cutoff values for score distances at $\sqrt{\chi^2_{1,.975}}$ and orthogonal distances at $[\text{median}(OD^{2/3}) + \text{MAD}(OD^{2/3})\Phi^{-1}(0.975)]^{3/2}$, where $\Phi(.)$ is the standard normal cumulative distribution function. These cutoffs are marked by red lines in the diagnostic plots in figure \ref{fig:distplot}. The classical PCA detects only 1 of the 6 outliers (left panel). Compared to this the diagnostic plot corresponding to depth PCA in the right panel correctly detects all 6 outlying points, which have much higher score distances and orthogonal distances than other points. Interestingly, the arrangement of outlying points here is very similar to that in the diagnostic plot corresponding to ROBPCA, which can be found in figure 5(b) of \cite{hubert05}.


\begin{figure}[t]
	\centering
		\includegraphics[height=6cm]{../Codes/octane_screeplot.png}
	\caption{(Left) Explained variance proportions for two types of PCA on octane data}
	\label{fig:screeplot}
\end{figure}
\begin{figure}[h]
	\centering
		\includegraphics[height=6.5cm]{../Codes/octane_distplot.png}
	\caption{(Left) Distance plots for two types of PCA on octane data}
	\label{fig:distplot}
\end{figure}
